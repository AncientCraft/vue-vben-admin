    ...
    <BasicTable @register="registerTable">
      <template #bodyCell="{ column, record }">
        ...
        <template v-if="column.key === 'spot'">
          <Switch v-model:checked="record.spot" @change="changeStatus(record)" />
        </template>
        <template v-if="column.key === 'fiat'">
          <Switch v-model:checked="record.fiat" @change="changeStatus(record)" />
        </template>
        <template v-if="column.key === 'option'">
          <Switch v-model:checked="record.option" @change="changeStatus(record)" />
        </template>
        <template v-if="column.key === 'market'">
          <Switch v-model:checked="record.market" @change="changeStatus(record)" />
        </template>
      </template>
    </BasicTable>
    <div>
